...
export function StudentProfileDrawer({ open, onClose, studentId }: StudentProfileDrawerProps) {
  ...
  const [assignWeakSkills, setAssignWeakSkills] = useState<string[]>([]);

  if (!studentId) return null;

  // Collect weak skills from radar data (where current < standard)
  const weakSkillsList = radarData
    .filter(item => item.current < item.standard)
    .map(item => item.skill);

  const handleRemind = () => {
    message.success(`已向 ${studentData.name} 发送催办通知`);
  };

  const handleAssignTask = (weakSkills: string[] = []) => {
    setAssignWeakSkills(weakSkills.length > 0 ? weakSkills : weakSkillsList);
    setAssignModalOpen(true);
  };

  const selectedStudentForAssign = [{
    id: studentData.id,
    ...
    <>
      ...
      >
        ...
        <div style={{ 
          ...
          <Button icon={<BellOutlined />} onClick={handleRemind}>催办</Button>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => handleAssignTask()}>指派任务</Button>
        </div>
      </Drawer>
      ...
      <TaskAssignModal
        ...
        selectedStudents={selectedStudentForAssign}
        weakSkills={assignWeakSkills}
      />
    </>
  );
}
