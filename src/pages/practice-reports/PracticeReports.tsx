...
const segmentTagConfig: Record<SegmentTag, { color: string; icon: string }> = {
  "可以优化": { color: "#faad14", icon: "💡" },
  "表现良好": { color: "#52c41a", icon: "✅" },
  "有错误": { color: "#ff4d4f", icon: "❌" },
};
...
const mockDetailData = {
  ...
  weaknesses: [
    { name: "客户洞察", detail: "未能深入挖掘客户真正的核心诉求和隐含需求" },
    { name: "方案呈现", detail: "解决方案的呈现缺乏结构化，关键数据支撑不足" },
  ],
  radarData: {
  ...
  segments: [
    {
      id: "seg1",
      label: "片段1",
      tag: "可以优化" as SegmentTag,
      turns: [
        { role: "ai" as const, content: "来了啊，唐宁远。昨晚直播间那事真是头疼，爆款连衣裙针音爆爆了，结果仓库系统显示缺货，实际天猫仓那边还有，这库存数据得同步才行。" },
        { role: "user" as const, content: "嗯，您这边的问题了解了。呢，首先呢就是天猫抖音多呢，平台库存不能同步。二是大促期期间，这个旺季拢收跟不上。三就是咱们的服饰烫贴标退费整理呢，现在的物流是做不了影响咱们二次销售，对吗？然后我们这边京东入仓以后..." },
        { role: "ai" as const, content: "嗯你说的有道理，那你们怎么保证多平台的数据同步呢？具体说说方案？" },
        { role: "user" as const, content: "好的，我们是通过交叉WMS系统对接到咱们的ERP系统上面，做到实时库存同步..." },
      ],
    ...
    },
    {
      id: "seg2",
      label: "片段2",
      tag: "有错误" as SegmentTag,
      turns: [
        { role: "ai" as const, content: "嗯，你说的这些肯定不错，不过，我还多多个水库荷叶储存流程资料？另外科技外包帮助出总告总" },
        { role: "user" as const, content: "哦，我们这里还有一些补充资料要跟您分享一下..." },
      ],
    ...
    },
    {
      id: "seg3",
      label: "片段3",
      tag: "表现良好" as SegmentTag,
      turns: [
        { role: "ai" as const, content: "嗯，上门约谈可以，不过我先对你报的价格有一些模块要确认，这几块钱下来其实很多..." },
        { role: "user" as const, content: "好的，我来给您详细拆解一下我们的价格构成。首先是仓储费用，按照SKU数量和存储面积计算..." },
        { role: "ai" as const, content: "这个拆解清楚多了，那合同周期和付款方式呢？" },
        { role: "user" as const, content: "我们标准合同是一年期，按月结算，首月可以提供试运营优惠..." },
      ],
      problemAnalysis: "",
      suggestions: "",
    },
...
export default function PracticeReports() {
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: "有效练习时长(分钟)", dataIndex: "duration", key: "duration", width: 140, align: "center",
      render: (v: number | null) => v ?? "-",
    },
    {
      title: "练习状态", dataIndex: "practiceStatus", key: "practiceStatus", width: 100,
      render: (v: string) => <Tag color={v === "练习完成" ? "green" : "blue"}>{v}</Tag>,
    },
    {
      title: "结果状态", dataIndex: "resultStatus", key: "resultStatus", width: 80, align: "center",
      render: (v: string) => v === "通过" ? <Tag color="success">通过</Tag> : <span>{v}</span>,
    },
    {
      title: "得分", dataIndex: "score", key: "score", width: 60, align: "center",
      render: (v: number | null) => v ?? "-",
    },
    {
      title: "结果", dataIndex: "summary", key: "summary", width: 200, ellipsis: true,
    ...
    <DashboardLayout title="练习报告" description="查看学员练习情况与详细报告">
      <div className="space-y-4">
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <span>练习计划：</span>
          <Select value={selectedPlan} onChange={setSelectedPlan} style={{ width: 320 }}
            options={mockPlans.map(p => ({ value: p.id, label: p.title }))} placeholder="选择练习计划" />
          <Input placeholder="搜索学员/ERP" prefix={<SearchOutlined />} value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)} style={{ width: 200 }} allowClear />
        </div>
        <div>
        ...
        <Card bodyStyle={{ padding: 0 }}>
          <Table columns={columns} dataSource={filteredData} rowKey="id"
            pagination={{ showSizeChanger: true, showTotal: (total) => `共 ${total} 条记录`, pageSize: 10 }}
            scroll={{ x: 1400 }} />
        </Card>
      </div>

      <Drawer title={null} open={detailOpen} onClose={() => setDetailOpen(false)} width="70vw" zIndex={1000}>
...
function ReportDetail({ data }: { data: typeof mockDetailData }) {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", background: "#fff7e6", padding: 20, borderRadius: 8 }}>
      ...
      <Card title="会话记录分析" size="small">
        <Collapse
          ...
          items={data.segments.map(seg => {
            const tagCfg = segmentTagConfig[seg.tag];
            return {
              key: seg.id,
              label: (
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontWeight: 600 }}>{seg.label}</span>
                  <Tag color={tagCfg.color}>{tagCfg.icon} {seg.tag}</Tag>
                </div>
              ),
              children: (
                <div className="space-y-4">
                  {seg.turns.map((turn, i) => (
                    <div key={i} style={{
                      padding: 12,
                      background: turn.role === "ai" ? "#fff7e6" : "#f6ffed",
                      borderRadius: 8,
                      borderLeft: `3px solid ${turn.role === "ai" ? "#fa8c16" : "#52c41a"}`,
                    }}>
                      <div style={{ fontWeight: 600, color: turn.role === "ai" ? "#fa8c16" : "#52c41a", marginBottom: 4 }}>
                        {turn.role === "ai" ? "AI 陪练" : "学员"}
                      </div>
